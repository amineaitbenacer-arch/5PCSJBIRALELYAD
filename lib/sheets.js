function sheetUrl() {
    try {
        ...
        const match = raw.match(/^GOOGLE_SHEETS_WEBHOOK=(.+)$/m);
...
async function updateOrderPhoneInSheet(orderId, phone, previousPhone) {
    const result = await postToSheet({
        action: 'updatePhone',
        orderId: String(orderId),
        phone: String(phone || '').replace(/\D/g, ''),
        previousPhone: String(previousPhone || '').replace(/\D/g, '')
    });
    if (result.skipped) return { ok: false, skipped: true };
